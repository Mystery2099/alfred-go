...
<div class="space-y-2">
  {#each app.categories.filter((category) => tools.some((tool) => tool.categoryId === category.id)) as category}
    ...
    <section class="overflow-hidden rounded-2xl bg-surface shadow-sm ring-1 ring-border">
      <button
        class="flex w-full items-center gap-3 px-4 py-4 text-left transition hover:bg-muted"
        onclick={() => toggleCategory(category.id)}
        aria-expanded={isOpen}
      >
        <span class="grid h-8 w-8 place-items-center rounded-xl bg-campus-blue text-sm text-white">{app.categoryGlyph(category.id)}</span>
        ...
        <ChevronRight class="h-4 w-4 text-text-soft transition {isOpen ? 'rotate-90' : ''}" />
      </button>
      {#if isOpen}
        <div class="px-4 pb-4">
          {#each tools.filter((tool) => tool.categoryId === category.id) as tool}
            <a href="/tools/{tool.id}" class="flex w-full items-center gap-3 rounded-xl px-2 py-3 text-left hover:bg-muted">
              <span class="grid h-7 w-7 shrink-0 place-items-center text-text-muted">
